...
- DiD spec: vol_it = α_i + λ_t + β·(banned_i × post_t) + X_it + ε_it
  ...
  - FE: unit and time. Cluster SEs at COMMODITY level (few clusters →)
...
- C1 CONFIRMED only if: same sign + magnitude within [4%, 16%] band + p<0.05 under + survives placebo-date test (fake ban at Dec 2019 shows nothing)
...
inconsistency in basis analysis; documented verification protocol" — strong research-integrity
